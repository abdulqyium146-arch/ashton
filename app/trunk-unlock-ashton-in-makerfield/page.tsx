...
export const metadata: Metadata = {
  ...
  description: 'Boot won\'t open or keys locked in the boot in Ashton-in-Makerfield? We access vehicle boots without damage — failed remote, mechanical fault, or locked inside. Call 07711 341668.',
...
const faqs = [
  ...
  {
    question: 'My keys are locked in the boot — can I still get my car open?',
  ...
  },
  {
    ...
    answer: 'No — we only use non-destructive methods. We will not drill, force, or use destructive techniques unless every non-destructive option has been exhausted and you have explicitly agreed. In practice, we achieve successful non-destructive access in the vast majority of boot-unlock situations.',
  },
  {
    question: 'What if the boot lock has mechanically failed, not just stuck?',
    answer: 'Mechanical boot lock failures — where the latch isn\'t releasing despite the remote or button operating — are usually caused by a failed actuator or a broken latch cable. We can diagnose and often repair these on-site. If a replacement part is required, we\'ll advise on sourcing and a follow-up visit.',
  },
  {
    question: 'How much does boot unlocking cost in Ashton-in-Makerfield?',
    answer: 'Boot unlocking as part of a car lockout starts from £60 (daytime). If the boot lock mechanism requires additional repair work, that is quoted separately before any additional work begins.',
  },
]
...
export default function TrunkUnlockAshtonPage() {
  ...
  const schemas = [
    serviceSchema('Boot & Trunk Unlocking Ashton-in-Makerfield', 'Professional boot and trunk unlocking in Ashton-in-Makerfield. Non-destructive access — failed remote, mechanical fault, or keys locked inside.', pageUrl),
    faqSchema(faqs),
    ...
    webPageSchema({ url: pageUrl, title: 'Boot & Trunk Unlocking Ashton-in-Makerfield | Mobile', description: 'Boot and trunk unlocking in Ashton-in-Makerfield. Non-destructive, mobile service — all situations covered.', type: 'WebPage' }),
  ]
    ...
    <>
      ...
      <section className="bg-gradient-to-br from-brand-navy to-brand-blue section-padding" aria-labelledby="page-heading">
        <div className="container-max max-w-4xl">
          <div className="inline-flex items-center gap-2 bg-green-500/20 border border-green-500/30 text-green-400 text-sm font-semibold px-4 py-2 rounded-full mb-6">
            ...
            Boot &amp; Trunk Unlocking — Ashton-in-Makerfield &amp; Wigan
          </div>
          ...
          <p className="text-xl text-white/70 mb-4 leading-relaxed">
            ...
            &amp; Key provides mobile boot and trunk unlocking across Ashton-in-Makerfield and Wigan —
      ...
      <section className="section-padding bg-white">
        <div className="container-max max-w-4xl mx-auto prose-custom">
          ...
          <p>
            A locked or stuck boot creates a frustrating situation — particularly when your keys are
          ...
          <p>
            ...
            there is no spare, we first open the vehicle door using standard non-destructive entry —
            the same technique used for a regular car lockout. Once inside, we operate the internal
            boot release to retrieve the keys. The whole process typically takes 20–40 minutes
            from arrival.
          </p>
          ...
          <p>
            Some vehicles — particularly older saloons and certain sports cars — don&apos;t have
      ...
      <CTASection variant="orange" heading="Boot Won't Open in Ashton-in-Makerfield?" subtext="Call Ashton Lock & Key — non-destructive boot access, 24/7, no call-out fee." />
        ...
        <div className="container-max">
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4">
            {[
              ...
              { title: 'Emergency Auto Locksmith', desc: '24/7 emergency — any vehicle access situation.', href: '/emergency-auto-locksmith-ashton-in-makerfield' },
